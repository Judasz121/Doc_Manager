...
function BuildLimitQuery($limit, $filter)
{
    ...
    $sql = "SELECT COUNT(*) FROM documents WHERE addedBy='" . $_SESSION['login'] . "' '".$filter."' AND progress != 'N' AND progress != 'W' AND progress != 'WP'";
...
function getAllDocumentsAssignedToUser(string $login, string $orderBy, string $order, string $limit, string $filter): array
{
	...
	$sql = 'SELECT * FROM documents WHERE addedBy="' . $login . '" ' . $filter . ' AND progress != "N" AND progress != "W" AND progress != "WP" AND progress != "P"  AND progress != "Z" AND progress != "PP" ORDER BY ' . $orderBy . ' ' . $order . $limit;
