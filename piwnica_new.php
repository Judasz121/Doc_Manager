<?php
	...
	require_once 'piwnicaFunctions_new.php';
	...
	while($user = $result -> fetch_assoc()){
		if($user['userName'] == $_SESSION['login'] && !( $user['accountType'] == "archiwum" || $user['accountType'] == "korespondencja"  || $user['accountType'] == "admin"))
			header('Location: login.php');
	}	
...
function copyToClip(str) {
  ...
  document.removeEventListener("copy", listener);
	...
	<?php include 'components/filtr_piwnica.php';?> <!-----< FILTR --->	
	<div class="biperContainer">
		...
		<tr>
			<td>WP</td>
			<td id="WPDocsAmount"></td>
		</tr>
		<tr>
			<td>NP</td>
			<td id="NPDocsAmount"></td>
		</tr>
		</tbody>
		</table>
			<div id="PPalert">
			...
			<div id="WPalert">
				<div class="led-yellow"></div> Nowy dokument WP <i class="icon-cancel " style="float: right; cursor: pointer;"></i>
			</div>
			<div id="NPalert">
				<div class="led-yellow"></div> Nowy dokument NP <i class="icon-cancel " style="float: right; cursor: pointer;"></i>
			</div>
			<div id="FIREalert">
...
?>
		...
		<?PHP
			...
			<div class="doc item '; $p = $doc['progress'];
				...
				<div class="docSpace"  style="white-space: nowrap">';
				if ($doc['progress'] === "NP" || $doc['progress'] === "N" || $doc['progress'] === "WP" || $doc['progress'] === "W") { echo '
					<label>
					...
					<label>';
					if ($doc['docSpace'] === "pracownik" ) {echo'
					...
					<label>';
					if ($doc['docSpace'] === "kierownik" ) {echo'
					<input type="radio" name="docSpace['.$currId.']" id="id'.$currId.'kierownik" value="kierownik" checked="checked" onclick="javascript: return false;" >
					';
					}else 
						echo'
						&nbsp;X&nbsp;
					</label>';
				}elseif ($doc['progress'] === "P" || $doc['progress'] === "Z") { echo '
					<label>
						&nbsp;X&nbsp;|
					</label>
					<label>
				...
				elseif ($doc['progress'] === "P" && ($doc['docSpace'] === "archiwum1" || $doc['docSpace'] === "archiwum2")) { echo '
					<label>
						&nbsp;>> !&nbsp;
					</label>
					<label>
				...
				}elseif ($doc['progress'] === "PP" && $doc['docSpace'] === "archiwum1") { echo '
					<label>';
					if ($doc['docSpace'] === "archiwum1" ) {echo'
				...
				}elseif ($doc['progress'] === "PP" && $doc['docSpace'] === "archiwum2") { echo '
					<label>
						&nbsp;X&nbsp;|
					</label>
					<label>';
					if ($doc['docSpace'] === "archiwum2" ) {echo'
				...
				<div class="progress">
					<select name="progress['.$currId.']">';
						...
						<option value="N"'; if($doc['progress'] === "N") echo 'selected'; echo'>N</option>';
						}elseif ($doc['progress'] === "P") { echo'
						<option value="P"'; if($doc['progress'] === "P") echo 'selected'; echo'>P</option>';
						}elseif ($doc['progress'] === "NP") { echo'
						<option value="N"'; if($doc['progress'] === "N") echo 'selected'; echo'>N</option>';
						}elseif ($doc['docImportance'] === "Zagr-N" || $doc['docImportance'] === "art.97 kpa") { echo'
						<option value="N"'; if($doc['progress'] === "N") echo 'selected'; echo'>N</option>
						<option value="Z"'; if($doc['progress'] === "Z") echo 'selected'; echo'>Z</option>';
						}elseif ($doc['progress'] === "WP") { echo'
    ...
    <td><input type="text" size="10" name="comments['.$currId.']" value="'.$doc['comments'].'" /></td>
		...
		<button class="managerkp" onclick="copyToClip(document.getElementById(\'row'.++$idrow.'\').innerHTML)">K</button>
			...
			<div id=row'.$idrow.' style="display:none"> Sprawa Nr. '.$doc['clientId'].'  '.$doc['clientName'].'  '.$doc['clientSurname'].'. Okres '.$doc['yearHired'].', Rodzaj '.$doc['docType'].' Przydzielono: '.$doc['workerAsigned'].'</div>
				...
				echo '
				<script>
					if(document.getElemetById('.$doc['docImportance'].') != null)
						document.getElemetById('.$doc['docImportance'].').checked = true;
					if(document.getElemetById('.$doc['docType'].') != null)
