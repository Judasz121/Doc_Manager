<?php
	...
	if($dontSave == false && isset($_POST['save'])) {
		...
		{
			//echo 'fullName before calling function -> '; var_dump($_POST['newUserFullname']); echo'<br>';
	...
	foreach($users as $user){
		...
		<tr>
			<td>
				...
				<input type="text"  size="5" name="userName['.$currId.']" value="'.$user['userName'].'"/> | 
			</td>
			<td><input type="text" size="27" name="userFullname['.$currId.']" value="'.$user['userFullname'].'"/> | </td>
			<td><input type="text" size="5" name="userPassword['.$currId.']" value="'.$user['userPassword'].'"/> | </td>
			<td>
			...
			<td><input type="text" size="2" name="userWorkerGroup['.$currId.']" value="'.$user['workerGroup'].'"/> | </td>
