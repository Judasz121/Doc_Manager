...
{
    $dateMinus90days = date('Y-m-d H:m:s', strtotime("-180 days"));
    $query = ' WHERE dateModified <= "'.$dateMinus90days.'"AND progress LIKE "N" AND docImportance LIKE "art.97 kpa"';
...
function SaveChanges(array $documentIds, array $clientNum, array $name, array $surName, array $yearHired, array $dateWorkStart, array $dateWorkEnd, array $docType, array $docImportance, array $progress, array $asignedTo, array $comments, $saveSelected, $deleteSelected, bool $saveOnlySelected)
{
    global $conn;
    ...
    foreach ($documents as $doc) {
        ...
        $query = 'UPDATE documents SET clientId = "' . $clientId . '", clientName="' . $clientName . '", clientSurname="' . $clientSurname . '", yearHired="' . $hiredStart . '",
				workStartDate="' . $dateStart . '", workEndDate="' . $workEndDate . '", docType="' . $docTypeDb . '",docImportance="' . $docImportanceDb . '", progress="' . $progressDb . '",
...
function BuildLimitQuery($limit, $filter)
{
    ...
    if ($_SESSION['numofPages'] + 1 < $_GET['page']) {
        ...
        header('Location: manager.php?page=1' . $query);
    }
...
{
    global $conn;
    $query = "insert into archive (clientId, clientName, clientSurname, yearHired, workStartDate, workEndDate, docType, docImportance, progress, workerAsigned, comments, dateModified, userModified, userIpModified, addedBy, addedTime)
(select clientId, clientName, clientSurname, yearHired, workStartDate, workEndDate, docType, docImportance, progress, workerAsigned, comments, dateModified, userModified, userIpModified, addedBy, addedTime 
