...
echo $_REQUEST['userName']. '  '.$_REQUEST['userFullname'].'
...
<tr>
	...
	<th>
		<a class="workerTableSortLink" id="?workersTableOrderby=docSpace&workersTableOrder=DESC' . BuildSortQueryForWorkersTable("workersTableOrderby") . '"> 
		<span>'.$docSpace.'</span>';
		if($_REQUEST['workersTableOrderby'] == "docSpace" && $_REQUEST['workersTableOrder'] == 'ASC') echo '<i class="icon-up-dir"></i>'; 
		elseif($_REQUEST['workersTableOrderby'] == "docSpace" && $_REQUEST['workersTableOrder'] == 'DESC') echo '<i class="icon-down-dir"></i>';
		echo'
	</th>
	<th>
		<a class="workerTableSortLink" id="?workersTableOrderby=MMM&workersTableOrder=DESC' . BuildSortQueryForWorkersTable("workersTableOrderby") . '"> 
		<span>'.$ogien.'</span>';
		if($_REQUEST['workersTableOrderby'] == "MMM" && $_REQUEST['workersTableOrder'] == 'ASC') echo '<i class="icon-up-dir"></i>'; 
		elseif($_REQUEST['workersTableOrderby'] == "MMM" && $_REQUEST['workersTableOrder'] == 'DESC') echo '<i class="icon-down-dir"></i>';
		echo'
	</th>
	<th>
...
echo'<br><div>
<table id="workerDocTypesTablea">
<thead>


<tbody>';

	$sql = 'SELECT COUNT(*) FROM documents WHERE workerAsigned="'.$_REQUEST['userName'].'"';
	$result4 = mysqli_query($conn, $sql);
	$numOfRows = mysqli_fetch_assoc($result4)['COUNT(*)'];
	
	
	echo'
	<tr>
		<td>Wszytskich teczek: </td>
		<td>'.$numOfRows.'</td>
	</tr>
...
if($result == false){
	...
	echo'<br>WITH SQL: '.$sql;
}

while($row = $result -> fetch_assoc()){
	$docType = $row['docType'];
	
	$sql = 'SELECT COUNT(*) FROM documents WHERE docType="'.$docType.'" AND workerAsigned="'.$_REQUEST['userName'].'"';
	$result1 = mysqli_query($conn, $sql);
	$amount = mysqli_fetch_array($result1)[0];
...
while($row = $result -> fetch_assoc()){
$docImportance = $row['docImportance'];

$sql = 'SELECT COUNT(*) FROM documents WHERE docImportance="'.$docImportance.'" AND workerAsigned="'.$_REQUEST['userName'].'"';
$result1 = mysqli_query($conn, $sql);
$amount = mysqli_fetch_array($result1)[0];
...
while($row = $result -> fetch_assoc()){
$progress = $row['docstatus'];

$sql = 'SELECT COUNT(*) FROM documents WHERE progress="'.$progress.'" AND workerAsigned="'.$_REQUEST['userName'].'"';
$result2 = mysqli_query($conn, $sql);
$amount = mysqli_fetch_array($result2)[0];
...
$sql = 'SELECT COUNT(*) FROM documents WHERE yearHired="'.$yearHired.'" AND workerAsigned="'.$_REQUEST['userName'].'"';
$result6 = mysqli_query($conn, $sql);
$amount = mysqli_fetch_array($result6)[0];
