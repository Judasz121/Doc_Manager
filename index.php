<?php
	...
	while($user = $result -> fetch_assoc()){
		if($user['userName'] == $_SESSION['login'] && !( $user['accountType'] == "admin" || $user['accountType'] == "pracownik" || $user['accountType'] == "koordynator" || $user['accountType'] == "dyrektor" || $user['accountType'] == "archiwum" || $user['accountType'] == "korespondencja" ))
			header('Location: login.php');
	}
	...
	//	echo "Błąd: " . $result . "<br>" . $query . "<br>" .mysqli_error($conn);
		...
		if($_SESSION['accType'] == "pracownik" || $_SESSION['accType'] == "koordynator"){
			$worker = "NN";
			$progress = "NP";
		}
		else{
		...
		"'.$_POST['docImportance'].'", "'.$_POST['docSpace'].'", "'.$_POST['MMM'].'", "'.$progress.'", "'.$worker.'","'.$_POST['comments'].'", "'.$date.'", "'.$_SESSION['login'].'", "'.getUserIpAddr().'", "'.$_SESSION['login'].'", "'.$date.'")';
		if(!isset($error))
			$result = mysqli_query($conn, $query);
		...
		if ($result === TRUE && !isset($error)) {
				...
				// ARCHIVE
				$query = 'SELECT * FROM documents WHERE clientId = "'.$_POST['clientNum'].'" AND clientName="'.$_POST['name'].'" AND clientSurname="'.$_POST['surName'].'" AND yearHired="'.$_POST['yearHired'].'" AND workStartDate="'.$_POST['dateWorkStart'].'" AND workEndDate="'.$_POST['dateWorkEnd'].'" AND docType="'.$_POST['docType'].'" AND docImportance="'.$_POST['docImportance'].'" AND docSpace="'.$_POST['docSpace'].'" AND MMM="'.$_POST['MMM'].'" AND progress="'.$progress.'"
				AND workerAsigned="'.$worker.'" AND comments="'.$_POST['comments'].'" AND userModified="'.$_SESSION['login'].'"';
				
				$result = mysqli_fetch_array(mysqli_query($conn, $query));
				if($result == false) {
					echo 'Nie wprowadzono do logów</br>ERROR: '.mysqli_error($conn).' </br>WITH SQL: '.$query.'</br>';
				}else{
					$query = 'INSERT INTO archive VALUES(NULL, "'.$_POST['clientNum'].'", "'.$_POST['name'].'", "'.$_POST['surName'].'", "'.$_POST['yearHired'].'", "'.$_POST['dateWorkStart'].'", "'.$_POST['dateWorkEnd'].'", "'.$_POST['docType'].'", "'.$_POST['docImportance'].'", "'.$_POST['docSpace'].'", "'.$_POST['MMM'].'", "'.$result['progress'].'", "'.$result['workerAsigned'].'","'.$_POST['comments'].'", "'.$date.'", "'.$_SESSION['login'].'", "'.getUserIpAddr().'", "'.$result['addedBy'].'", "'.$result['addedTime'].'" )';
					$result = mysqli_query($conn, $query);
				...
				while($row = $result -> fetch_assoc()){
					echo '
						<label>
							...
							<span >'.$row['docImportance'].'</span>
						</label>';
				}
			?>
		</div>
		<div class="wybor">
			<span><?php echo $docSpace1;?>:.................................</span><a class="tooltip" href="#">(?)<span class="classic"><font size="2"><?php echo $opis8; ?></font></span></a><br>
			<label>
				<input type="radio" name="docSpace" id="" value="pracownik"  checked="checked"/>
				<span><img src="ikony/hand.png" height="15px"/></span>
			</label>

		</div>
				<?PHP echo mysqli_real_escape_string($conn, '<i class="icon-fire"></i>ogien1'); ?>
		<div>
			...
			<?php
				...
				while($row = $result -> fetch_assoc()){
					...
					{
						echo '<option style="color:black;" value="'.$row['MMM'].'"'; if(($_POST['MMM'] == $row['MMM'] && $errorEmpty) || $firstRow == true) echo ' selected '; echo' >'.$row['MMM'].'</option>';
					}
					$firstRow = false;
				}
			?>
...
<button type="button" onclick="AddTo<?php echo $uwagi;?>Input('IT. ');">Włochy</button>
			...
			<textarea cols="30" rows="5" maxlength="100"id="<?php echo $uwagi;?>" type="text" name="comments" /></textarea>
					</div>-----------------------------------------------------------<br>
	...
	<div class="doc item '; $p = $doc['progress'];
		if($p == "N") echo 'blue'; elseif($p == "P") echo 'yellow'; elseif($p == "Z") echo 'orange'; elseif($p == "W") echo 'green'; elseif($p == "NP") echo 'npcolor';  echo'">
		<div class="clientNum">
			'.$doc['clientId'].'
		</div>
		<div class="clientName">
